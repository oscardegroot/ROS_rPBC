...
EdgeDirect::EdgeDirect(int i, int j, Eigen::MatrixXd gain_set, int l_set, Eigen::VectorXd r_star_set)
...
Eigen::VectorXd EdgeDirect::calculateControls(Eigen::VectorXd s_in, Eigen::VectorXd r_i){

	return gain_tau.block(0, 0, l, l) * s_in + gain_tau.block(0, l, l, l)*r_i;
}

// Retrieve the new wave from the scattering transformation
Eigen::VectorXd EdgeDirect::calculateWaves(Eigen::VectorXd s_in, Eigen::VectorXd r_i){
